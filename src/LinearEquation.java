public class LinearEquation {
	...
	public String equation() {
		...
		// edge case that i dont want to deal with using abstraction
		if (m == 0.0) {
			return String.format("y = %d", y1);
		}

		String eq = "y = ";

		// slope -> string calculations
		int deltaY = y2 - y1;
		int deltaX = x2 - x1;
		...
		if (Math.abs(m) != 1) {
			if (Math.floor(m) == m) {
				// whole number slope
				mVal = String.format("%d",
					(int)Math.abs(m));
			} else {
				// fractional slope
				mVal = String.format("%d/%d",
					Math.abs(deltaY),
					Math.abs(deltaX));
			}
		}
		...
		// y intercept -> string calculations
		if (b != 0) {
			String bSign = "+";
			if (b < 0)
				bSign = "-";
	...
	public String lineInfo() {
		String info = "";
		info += "The two points are: "
			+ String.format("(%d, %d)", x1, y1) + " and "
			+ String.format("(%d, %d)", x2, y2) + "\n";
		info += "The equation of the line between these points is: "
			+ equation() + "\n";
		info += "The slope of this line is: "
			+ slope() + "\n";
		info += "The y-intercept of the line is: "
			+ yIntercept() + "\n";
		info += "The distance between the two points is: "
			+ distance();
		return info;
	}
}
